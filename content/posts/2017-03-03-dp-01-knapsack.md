...
tags:
  - 动态规划
excerpt: "通过经典0-1背包问题介绍动态规划思想，利用二维数组实现最优解求解，理解状态转移方程的核心原理。"
aiSummary: "本文以0-1背包问题为切入点介绍动态规划算法。以书包容量限制下选取商品使价值最大化的问题为例，通过穷举法引出动态规划的必要性。核心解法是使用二维数组 dp[i][j] 表示前 i 件物品在容量 j 下的最大价值，状态转移方程为：dp[i][j] = max(dp[i-1][j], dp[i-1][j-weight[i]] + value[i])。文章详细分析了状态转移的推导过程，帮助读者理解动态规划的最优子结构与无后效性特征。"
---
...
因为有初始最小解(为0)的情况, 矩阵应该为`matrix[N+1][W+1]`;

也就是N+1行, W+1列的二维数组;

将矩阵结合上述推导, 我们分出三种情况(item:物品个数, weight:背包承重数):

1. 初始解: item=0, weight=0
...
1. 能否放物品2 ? 可以啊! 物品2重量为4.
2. 不放物品2是否价值更大? 不是阿! 不放价值为0(看前一行数据)
3. 放入物品2后还能放? 不能啊! 放了之后背包剩余承重为0了
...
3. 放上物品3之后还能再放么? 能啊! 放完之后还剩余(10-2)=8呢
