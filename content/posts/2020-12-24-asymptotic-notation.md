...
tags:
  - 数学
excerpt: "介绍算法分析中的渐近符号：O（大O上界）、Ω（渐近下界）、Θ（紧确界）及其性质和等价性。"
aiSummary: "本文系统介绍了算法复杂度分析中的渐近符号体系，包括 O（大O上界，表示渐近不超过）、Ω（渐近下界，表示渐近不低于）、Θ（紧确界，表示上下界相同）。文章详细阐述了非渐近紧确界的情况，三种符号的数学定义，以及 O、Ω、Θ 之间的性质关系和等价性判定，并提供了求渐近界的方法和实例分析。"
---
...
同样, 可以得到f(n) = Ω(n^2) 与 f(n) = θ(n^2), 方法类似, 不再赘述
